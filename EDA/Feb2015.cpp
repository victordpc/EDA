...
//void optimizar(int n, producto* v, int precioMax, int proteinaMin, bool* solucion);
//
//void ejercicio3(){
...
//    productos= new producto[numero];
//    comprados= new bool[numero];
//    llenarProductos(numero, productos);
//    cout << "Precio maximo "; cin >> precioMax;
//    cout << "Proteina minimo "; cin >> proteinaMin;
//    
//     optimizar(numero, productos, precioMax, proteinaMin, comprados);
//    
//}
//
...
//
//
//void optimizarVa(int n, int k, producto* v, int maxPrecio, int minProt, bool* solucion, int calorias, int proteinas, int precio, bool* mejorSolucion, int& mejorCalorias, int proteinasRestantes);
//
//bool esValida(int precio,int maxPrecio,int calorias,int mejorCalorias,int proteinas,int proteinasRestantes,int minProt);
//
//bool esValida(int precio,int maxPrecio,int calorias,int mejorCalorias,int proteinas,int proteinasRestantes,int minProt){
//    return (precio <= maxPrecio) && (calorias < mejorCalorias) && (proteinas + proteinasRestantes > minProt);
//}
//
//bool esResultado(int n, int k, int proteinas, int minProt){
//    return (k == n - 1) || (proteinas >= minProt);
//}
//
//void optimizarVa(int n, int k, producto* v, int maxPrecio, int minProt, bool* solucion, int calorias, int proteinas, int precio, bool* mejorSolucion, int& mejorCalorias, int proteinasRestantes){
//
...
//
//    precio             += v[k].precio;
//    calorias           += v[k].calorias;
//    proteinas          += v[k].proteinas;
//    proteinasRestantes -= v[k].proteinas;
//
//    if (esValida(precio, maxPrecio, calorias, mejorCalorias, proteinas, proteinasRestantes, minProt)){
//        if(esResultado(n, k, proteinas, minProt)){
//            if(calorias < mejorCalorias){
//                mejorCalorias=calorias;
//                int i = 0;
//                for (; i <= k; i++) mejorSolucion[i] = solucion[i];
//                for (; i < n; i++) mejorSolucion[i] = false;
//            }
//        }else{
//            optimizarVa(n, k+1, v, maxPrecio, minProt, solucion, calorias, proteinas, precio, mejorSolucion, mejorCalorias, proteinasRestantes);
//        }
//    }
//
//    proteinas -= v[k].proteinas;
...
//    int mejorCalorias  = 1;
//
//    bool* mejorSolucion = new bool[n];
//
//    for (int i = 0; i < n; i++) {
//        solucion[i] = false;
//        mejorSolucion[i] = false;
//
//        totalProteinas += v[i].proteinas;
//        mejorCalorias  += v[i].calorias;
//    }
//    optimizarVa(n, 0, v, precioMax, proteinaMin, solucion, 0, 0, 0, mejorSolucion, mejorCalorias, totalProteinas);
//
//    cout<< "Mejor calorias " << mejorCalorias << " comprados " << endl;
//    for (int i = 0; i<n; i++) {
//        if(mejorSolucion[i]) cout << "producto " << i << endl;
//    }
//}
//
//void llenarProductos(int n, producto* v){
