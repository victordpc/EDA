...
public:
	...
	*/
	...
	int ContarHojas() {
		int numeroHojas = 0;
		numHojas(_ra, numeroHojas);
		return numeroHojas;
	}

	int CalcularAltura() {
		int numeroAltura = 0;
		numAltura(_ra, numeroAltura);
		return numeroAltura;
	}
protected:
	...
	static void numAltura(Nodo *ra, int &numero) {
		if (ra == NULL)
			return;

		int izquierdo = 0, derecho =0;
		numAltura(ra->_iz, izquierdo);
		numAltura(ra->_dr, derecho);

		numero = (izquierdo >= derecho) ? izquierdo + 1 : derecho + 1;
	}

	static void numNodos(Nodo *ra, int &numero) {
		if (ra == NULL)
			return;

		++numero;
		numNodos(ra->_iz, numero);
		numNodos(ra->_dr, numero);
	}

	static void numHojas(Nodo *ra, int &numero) {
		if (ra == NULL)
			return;
		...
		if (ra->_iz == NULL && ra->_dr == NULL) {
			...
			numHojas(ra->_iz, numero);
			numHojas(ra->_dr, numero);
		}
	}
